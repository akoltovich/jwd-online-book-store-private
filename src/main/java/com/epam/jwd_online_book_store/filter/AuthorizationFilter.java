...
public class AuthorizationFilter extends HttpFilter {
    ...
    private static final List<String> COMMANDS_DENIED_FOR_USER = Arrays.asList(
            ADMIN_AREA,
            ADMIN_PAGE,
            ADD_BOOK,
            UNBAN_USER,
    ...
    private static final List<String> COMMANDS_DENIED_FOR_ADMIN = Arrays.asList(
            USER_AREA,
            USER_PAGE,
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (user == null) {
            long count = COMMANDS_NEEDED_AUTHORIZATION.stream().filter(s -> s.equals(request.getParameter("command"))).count();
            if (count > 0) {
                response.sendRedirect(PathToPages.SIGN_IN_PAGE_REDIRECT);
                return;
            }
        } else if (user.getRoleId() == 2) {
            long count = COMMANDS_DENIED_FOR_USER.stream().filter(s -> s.equals(request.getParameter("command"))).count();
            if (count > 0) {
                response.sendRedirect(PathToPages.ACCESS_DENIED_REDIRECT);
                return;
            }
        } else if (user.getRoleId() == 1) {
            long count = COMMANDS_DENIED_FOR_ADMIN.stream().filter(s -> s.equals(request.getParameter("command"))).count();
            if (count > 0) {
                response.sendRedirect(PathToPages.ACCESS_DENIED_REDIRECT);
                return;
            }
